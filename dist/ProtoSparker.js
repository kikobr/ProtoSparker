...
var PS = (function (exports) {
	...
	var getFillDefs, getMatrixTransform, getRootG, getUseDefs, getViewBox;
	  ...
	  if (!isFirefox && node.closest('[mask]')) {
	    ...
	    for (index = m = 0, len3 = ref1.length; m < len3; index = ++m) {
	      ...
	      if (child.nodeName === 'use' || child.nodeName === 'rect' || child.nodeName === 'path') {
	        ...
	        if (node.nodeName === 'g') {
	          if (node.children.length === 1 && node.children[0].nodeName === 'use') {
	          } else if (nodeBBox) {
	            childTx += nodeBBox.x;
	            childTy += nodeBBox.y;
	          }
	        } else if (node.parentNode.nodeName === 'g' && node.parentNode.children.length > 1) {
